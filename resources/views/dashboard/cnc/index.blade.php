        ...
        <div class="p-6 text-gray-900 dark:text-gray-100">
          ...
          <table id="example" class="cell-border compact stripe " style="width:100%">
            ...
            <tbody>
              @foreach ($cncs as $cnc)
                <tr>
                  <td class="break-words">{{ $cnc->proker }}</td>
                  <td class="break-words">{{ $cnc->renker }}</td>
                  <td class="break-words">{{ $cnc->target }}</td>
                  <td>{{ $cnc->janw1 == 'OK' ? '✔️' : $cnc->janw1 }}</td>
                  <td>{{ $cnc->janw2 == 'OK' ? '✔️' : $cnc->janw2 }}</td>
                  <td>{{ $cnc->janw3 == 'OK' ? '✔️' : $cnc->janw3 }}</td>
                  <td>{{ $cnc->janw4 == 'OK' ? '✔️' : $cnc->janw4 }}</td>
                  <td class="whitespace-nowrap px-4 py-2" style="text-align: center;">
                    @if ($cnc->hasil)
                      <div>
                        <a cncef="{{ Storage::url($cnc->hasil) }}" 
                          class="text-sm font-medium text-blue-600 dark:text-blue-500 hover:underline"
                          download="{{ Str::afterLast($cnc->hasil, '/') }}">
                          Sudah Upload
                        </a>
                      </div>
                    @else
                  ...
                  <td style="text-align: center;">{{ $cnc->achieve }}</td>
                  <td class="break-words" style="text-align: center;">{{ $cnc->ket }}</td>
                  <td style="text-align: center;">{{ $cnc->persen }}</td>
                  <td style="text-align: center; padding: 1.5rem 1.5rem 1.5rem 1.5rem;">
                    @role('user')
                      @if (empty($cnc->hasil))
                        <a cncef="{{ route('dashboard.cnc.edit', $cnc->id) }}"
