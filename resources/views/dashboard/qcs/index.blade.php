        ...
        <div class="p-6 text-gray-900 dark:text-gray-100">
          ...
          <table id="example" class="cell-border compact stripe " style="width:100%">
            ...
            <tbody>
              @foreach ($qcss as $qcs)
                <tr>
                  <td class="break-words">{{ $qcs->proker }}</td>
                  <td class="break-words">{{ $qcs->renker }}</td>
                  <td class="break-words">{{ $qcs->target }}</td>
                  <td>{{ $qcs->janw1 == 'OK' ? '✔️' : $qcs->janw1 }}</td>
                  <td>{{ $qcs->janw2 == 'OK' ? '✔️' : $qcs->janw2 }}</td>
                  <td>{{ $qcs->janw3 == 'OK' ? '✔️' : $qcs->janw3 }}</td>
                  <td>{{ $qcs->janw4 == 'OK' ? '✔️' : $qcs->janw4 }}</td>
                  <td class="whitespace-nowrap px-4 py-2" style="text-align: center;">
                    @if ($qcs->hasil)
                      <div>
                        <a qcsef="{{ Storage::url($qcs->hasil) }}" 
                          class="text-sm font-medium text-blue-600 dark:text-blue-500 hover:underline"
                          download="{{ Str::afterLast($qcs->hasil, '/') }}">
                          Sudah Upload
                        </a>
                      </div>
                    @else
                  ...
                  <td style="text-align: center;">{{ $qcs->achieve }}</td>
                  <td class="break-words" style="text-align: center;">{{ $qcs->ket }}</td>
                  <td style="text-align: center;">{{ $qcs->persen }}</td>
                  <td style="text-align: center; padding: 1.5rem 1.5rem 1.5rem 1.5rem;">
                    @role('user')
                      @if (empty($qcs->hasil))
                        <a qcsef="{{ route('dashboard.qcs.edit', $qcs->id) }}"
