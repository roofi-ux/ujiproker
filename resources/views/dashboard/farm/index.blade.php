          ...
          <table id="example" class="cell-border compact stripe " style="width:100%">
            ...
            <tbody>
              @foreach ($farms as $farm)
                <tr>
                  <td class="break-words">{{ $farm->proker }}</td>
                  <td class="break-words">{{ $farm->renker }}</td>
                  <td class="break-words">{{ $farm->target }}</td>
                  <td>{{ $farm->janw1 == 'OK' ? '✔️' : $farm->janw1 }}</td>
                  <td>{{ $farm->janw2 == 'OK' ? '✔️' : $farm->janw2 }}</td>
                  <td>{{ $farm->janw3 == 'OK' ? '✔️' : $farm->janw3 }}</td>
                  <td>{{ $farm->janw4 == 'OK' ? '✔️' : $farm->janw4 }}</td>
                  <td class="whitespace-nowrap px-4 py-2" style="text-align: center;">
                    @if ($farm->hasil)
                      <div>
                        <a farmef="{{ Storage::url($farm->hasil) }}" 
                          class="text-sm font-medium text-blue-600 dark:text-blue-500 hover:underline"
                          download="{{ Str::afterLast($farm->hasil, '/') }}">
                          Sudah Upload
                        </a>
                      </div>
                    @else
                  ...
                  <td style="text-align: center;">{{ $farm->achieve }}</td>
                  <td class="break-words" style="text-align: center;">{{ $farm->ket }}</td>
                  <td style="text-align: center;">{{ $farm->persen }}</td>
                  <td style="text-align: center; padding: 1.5rem 1.5rem 1.5rem 1.5rem;">
                    @role('user')
                      @if (empty($farm->hasil))
                        <a farmef="{{ route('dashboard.farm.edit', $farm->id) }}"
