        ...
        <div class="p-6 text-gray-900 dark:text-gray-100">
          ...
          <table id="example" class="cell-border compact stripe " style="width:100%">
            ...
            <tbody>
              @foreach ($gas as $ga)
                <tr>
                  <td class="break-words">{{ $ga->proker }}</td>
                  <td class="break-words">{{ $ga->renker }}</td>
                  <td class="break-words">{{ $ga->target }}</td>
                  <td>{{ $ga->janw1 == 'OK' ? '✔️' : $ga->janw1 }}</td>
                  <td>{{ $ga->janw2 == 'OK' ? '✔️' : $ga->janw2 }}</td>
                  <td>{{ $ga->janw3 == 'OK' ? '✔️' : $ga->janw3 }}</td>
                  <td>{{ $ga->janw4 == 'OK' ? '✔️' : $ga->janw4 }}</td>
                  <td class="whitespace-nowrap px-4 py-2" style="text-align: center;">
                    @if ($ga->hasil)
                      <div>
                        <a href="{{ Storage::url($ga->hasil) }}"
                          class="text-sm font-medium text-blue-600 dark:text-blue-500 hover:underline"
                          download="{{ Str::afterLast($ga->hasil, '/') }}">
                          Sudah Upload
                        </a>
                      </div>
                    @else
                  ...
                  <td style="text-align: center;">{{ $ga->achieve }}</td>
                  <td class="break-words" style="text-align: center;">{{ $ga->ket }}</td>
                  <td style="text-align: center;">{{ $ga->persen }}</td>
                  <td style="text-align: center; padding: 1.5rem 1.5rem 1.5rem 1.5rem;">
                    @role('ga')
                      @if (empty($ga->hasil))
                        <a href="{{ route('dashboard.ga.edit', $ga->id) }}"
