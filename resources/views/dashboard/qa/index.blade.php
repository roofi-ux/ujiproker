        ...
        <div class="p-6 text-gray-900 dark:text-gray-100">
          ...
          <table id="example" class="cell-border compact stripe " style="width:100%">
            ...
            <tbody>
              @foreach ($qas as $qa)
                <tr>
                  <td class="break-words">{{ $qa->proker }}</td>
                  <td class="break-words">{{ $qa->renker }}</td>
                  <td class="break-words">{{ $qa->target }}</td>
                  <td>{{ $qa->janw1 == 'OK' ? '✔️' : $qa->janw1 }}</td>
                  <td>{{ $qa->janw2 == 'OK' ? '✔️' : $qa->janw2 }}</td>
                  <td>{{ $qa->janw3 == 'OK' ? '✔️' : $qa->janw3 }}</td>
                  <td>{{ $qa->janw4 == 'OK' ? '✔️' : $qa->janw4 }}</td>
                  <td class="whitespace-nowrap px-4 py-2" style="text-align: center;">
                    @if ($qa->hasil)
                      <div>
                        <a href="{{ Storage::url($qa->hasil) }}"
                          class="text-sm font-medium text-blue-600 dark:text-blue-500 hover:underline"
                          download="{{ Str::afterLast($qa->hasil, '/') }}">
                          Sudah Upload
                        </a>
                      </div>
                    @else
                  ...
                  <td style="text-align: center;">{{ $qa->achieve }}</td>
                  <td class="break-words" style="text-align: center;">{{ $qa->ket }}</td>
                  <td style="text-align: center;">{{ $qa->persen }}</td>
                  <td style="text-align: center; padding: 1.5rem 1.5rem 1.5rem 1.5rem;">
                    @role('qa')
                      @if (empty($qa->hasil))
                        <a href="{{ route('dashboard.qa.edit', $qa->id) }}"
