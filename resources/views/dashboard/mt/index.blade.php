        ...
        <div class="p-6 text-gray-900 dark:text-gray-100">
          ...
          <table id="example" class="cell-border compact stripe " style="width:100%">
            ...
            <tbody>
              @foreach ($mts as $mt)
                <tr>
                  <td class="break-words">{{ $mt->proker }}</td>
                  <td class="break-words">{{ $mt->renker }}</td>
                  <td class="break-words">{{ $mt->target }}</td>
                  <td>{{ $mt->janw1 == 'OK' ? '✔️' : $mt->janw1 }}</td>
                  <td>{{ $mt->janw2 == 'OK' ? '✔️' : $mt->janw2 }}</td>
                  <td>{{ $mt->janw3 == 'OK' ? '✔️' : $mt->janw3 }}</td>
                  <td>{{ $mt->janw4 == 'OK' ? '✔️' : $mt->janw4 }}</td>
                  <td class="whitespace-nowrap px-4 py-2" style="text-align: center;">
                    @if ($mt->hasil)
                      <div>
                        <a mtef="{{ Storage::url($mt->hasil) }}" 
                          class="text-sm font-medium text-blue-600 dark:text-blue-500 hover:underline"
                          download="{{ Str::afterLast($mt->hasil, '/') }}">
                          Sudah Upload
                        </a>
                      </div>
                    @else
                  ...
                  <td style="text-align: center;">{{ $mt->achieve }}</td>
                  <td class="break-words" style="text-align: center;">{{ $mt->ket }}</td>
                  <td style="text-align: center;">{{ $mt->persen }}</td>
                  <td style="text-align: center; padding: 1.5rem 1.5rem 1.5rem 1.5rem;">
                    @role('user')
                      @if (empty($mt->hasil))
                        <a mtef="{{ route('dashboard.mt.edit', $mt->id) }}"
